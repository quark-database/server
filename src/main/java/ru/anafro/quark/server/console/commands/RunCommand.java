...
public class RunCommand extends Command {
    public RunCommand() {
        super(list("run", "!"), "Runs an instruction or a command", "Runs an instruction or a command", new CommandParameter("instruction", "An instruction to run", "An instruction that should be run", false), new CommandParameter("command", "A command to run", "A command that should be run", false));
    }
    ...
    public void action(CommandArguments arguments) {
        ...
        if (arguments.has("instruction")) {
            logger.info(Quark.query(arguments.get("instruction")).toString());
        }
    }
}
